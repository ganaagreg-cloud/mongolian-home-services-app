...
const SETTING_META: Record<string, { label: string; suffix: string; description: string }> = {
  ...
  late_cancel_fee:       { label: 'Хожуу цуцлах торгууль',      suffix: '₮',  description: 'Хугацаанаас хэтрэн цуцлахад тооцох торгууль' },
...
export default function MasterDataPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!sJson.success || !stJson.success || !dJson.success) { setPageState('error'); return }
  ...
  async function stToggle(id: number, current: boolean) {
    ...
    try {
      ...
      setStBusy(b => { const n = { ...b }; delete n[id]; return n })
    }
  }
  ...
  async function stCreate() {
    ...
    try {
      ...
      setStBusy(b => { const n = { ...b }; delete n['new']; return n })
    }
  }
  ...
  async function distToggle(id: number, current: boolean) {
    ...
    try {
      ...
      setDistBusy(b => { const n = { ...b }; delete n[id]; return n })
    }
  }
  ...
  async function distCreate() {
    ...
    try {
      ...
      setDistBusy(b => { const n = { ...b }; delete n['new']; return n })
    }
  }
  ...
  if (pageState === 'error') return (
    <div className="px-8 pt-8">
      <div className="flex flex-col items-center justify-center rounded-2xl bg-card py-16 shadow-sm">
        ...
        <button
          onClick={load}
          className="mt-4 flex h-10 items-center gap-2 rounded-2xl border border-border bg-card px-4 text-sm font-medium text-foreground shadow-sm transition-all hover:bg-card/80 active:scale-95"
        >
  ...
  return (
    <div className="space-y-8 px-8 pb-8 pt-8">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-foreground">Мастер дата</h1>
        <button
          onClick={load}
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-foreground">Тохиргоо</h2>
        </div>
        <div className="overflow-hidden rounded-2xl bg-card shadow-sm">
          {SETTING_KEYS.map((key, idx) => {
            const meta = SETTING_META[key]!
            const val  = settingDraft[key] ?? ''
            return (
              <div
                key={key}
                className={cn(
                  'flex items-center gap-4 px-5 py-4',
                  idx < SETTING_KEYS.length - 1 ? 'border-b border-border' : '',
                )}
              >
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="—"
                  />
                  <span className="w-6 text-sm text-muted-foreground">{meta.suffix}</span>
                </div>
              </div>
            )
          })}
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-foreground">Үйлчилгээний төрлүүд</h2>
          </div>
          {!stAddOpen && (
            <button
        ...
        <div className="overflow-hidden rounded-2xl bg-card shadow-sm">
          ...
          {stAddOpen && (
            <div className="grid grid-cols-[2.5rem_1fr_6rem_5rem] gap-3 items-center border-b border-border bg-primary/5 px-5 py-3">
              ...
              <input
                ...
                onKeyDown={e => e.key === 'Enter' && stCreate()}
                placeholder="Үйлчилгээний нэр"
                className="rounded-2xl border border-border bg-background px-3 py-1.5 text-sm text-foreground outline-none focus:border-primary"
              />
              <span />
              <div className="flex justify-end gap-1">
                <button
                  disabled={!!stBusy['new']}
                  onClick={stCreate}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-success/10 text-success transition-all hover:bg-success/20 active:scale-95 disabled:opacity-50"
                >
                  <Check className="h-4 w-4" />
                </button>
                <button
                  onClick={() => { setStAddOpen(false); setStNewName('') }}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/50 text-muted-foreground transition-all hover:bg-muted active:scale-95"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
          ...
          {serviceTypes.length === 0 && (
            <p className="px-5 py-8 text-center text-sm text-muted-foreground">Үйлчилгээний төрөл байхгүй байна</p>
          )}
          ...
          {serviceTypes.map((st, idx) => (
            <div
              key={st.id}
              className={cn(
                'grid grid-cols-[2.5rem_1fr_6rem_5rem] gap-3 items-center px-5 py-3',
                idx < serviceTypes.length - 1 ? 'border-b border-border' : '',
              )}
            >
              ...
              {stEditId === st.id ? (
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') stSaveEdit(st.id); if (e.key === 'Escape') setStEditId(null) }}
                  className="rounded-2xl border border-primary bg-background px-3 py-1.5 text-sm text-foreground outline-none"
                />
              ) : (
                <span className={cn('text-sm font-medium', !st.is_active && 'text-muted-foreground line-through')}>
                  {st.name_mn}
                </span>
              )}
              ...
              <div className="flex justify-center">
                <button
                  disabled={!!stBusy[st.id]}
                  onClick={() => stToggle(st.id, st.is_active)}
                  className={cn(
                    'rounded-full px-2 py-0.5 text-[11px] font-medium transition-all active:scale-95 disabled:opacity-50',
                    st.is_active
              ...
              <div className="flex justify-end gap-1">
                {stEditId === st.id ? (
                  ...
                  <button
                    onClick={() => { setStEditId(st.id); setStDraft(st.name_mn) }}
                    className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/50 text-muted-foreground transition-all hover:bg-muted active:scale-95"
                  >
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-foreground">Дүүргүүд</h2>
          </div>
          {!distAddOpen && (
            <button
        ...
        <div className="overflow-hidden rounded-2xl bg-card shadow-sm">
          ...
          {distAddOpen && (
            <div className="grid grid-cols-[2.5rem_1fr_6rem_5rem] gap-3 items-center border-b border-border bg-primary/5 px-5 py-3">
              ...
              <input
                autoFocus
                value={distNewName}
                onChange={e => setDistNewName(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && distCreate()}
              ...
              <div className="flex justify-end gap-1">
                ...
                <button
                  onClick={() => { setDistAddOpen(false); setDistNewName('') }}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/50 text-muted-foreground transition-all hover:bg-muted active:scale-95"
                >
                  <X className="h-4 w-4" />
                </button>
          ...
          {districts.length === 0 && (
            <p className="px-5 py-8 text-center text-sm text-muted-foreground">Дүүрэг байхгүй байна</p>
          )}
          ...
          {districts.map((district, idx) => (
            <div
              ...
              className={cn(
                'grid grid-cols-[2.5rem_1fr_6rem_5rem] gap-3 items-center px-5 py-3',
                idx < districts.length - 1 ? 'border-b border-border' : '',
              )}
            >
              ...
              {distEditId === district.id ? (
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') distSaveEdit(district.id); if (e.key === 'Escape') setDistEditId(null) }}
                  className="rounded-2xl border border-primary bg-background px-3 py-1.5 text-sm text-foreground outline-none"
                />
              ) : (
                <span className={cn('text-sm font-medium', !district.is_active && 'text-muted-foreground line-through')}>
                  {district.name_mn}
                </span>
              )}
              ...
              <div className="flex justify-center">
                <button
                  disabled={!!distBusy[district.id]}
                  onClick={() => distToggle(district.id, district.is_active)}
                  className={cn(
                    'rounded-full px-2 py-0.5 text-[11px] font-medium transition-all active:scale-95 disabled:opacity-50',
                    district.is_active
                      ? 'bg-success/10 text-success hover:bg-success/20'
              ...
              <div className="flex justify-end gap-1">
                {distEditId === district.id ? (
                  <>
                    <button
                      disabled={!!distBusy[district.id]}
                      onClick={() => distSaveEdit(district.id)}
                      className="flex h-8 w-8 items-center justify-center rounded-full bg-success/10 text-success transition-all hover:bg-success/20 active:scale-95 disabled:opacity-50"
                    >
                      <Check className="h-4 w-4" />
                    </button>
                    <button
                      onClick={() => setDistEditId(null)}
                      className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/50 text-muted-foreground transition-all hover:bg-muted active:scale-95"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  ...
                  <button
                    onClick={() => { setDistEditId(district.id); setDistDraft(district.name_mn) }}
                    className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/50 text-muted-foreground transition-all hover:bg-muted active:scale-95"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
