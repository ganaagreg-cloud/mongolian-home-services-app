...
export const TABLES: string[] = [
  ...
  // WebP thumbnail URLs for before/after photos (generated by sharp on upload)
  `ALTER TABLE orders ADD COLUMN IF NOT EXISTS before_thumb_url TEXT`,
  `ALTER TABLE orders ADD COLUMN IF NOT EXISTS after_thumb_url  TEXT`,

  // Account lockout: track consecutive failed login attempts per user
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS failed_login_attempts INT NOT NULL DEFAULT 0`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS locked_until TIMESTAMPTZ`,

  // PDPL audit trail — user_id intentionally has no ON DELETE CASCADE so records
  // survive account erasure (financial/legal retention requirement)
  ...
  // Dispute state machine: add resolution metadata columns + backfill legacy 'resolved' rows
  `ALTER TABLE disputes ADD COLUMN IF NOT EXISTS resolution_note TEXT`,
  `ALTER TABLE disputes ADD COLUMN IF NOT EXISTS reviewed_by INTEGER REFERENCES users(id)`,
  // Backfill: old 'resolved' = escrow released to worker → map to new terminal state
  `UPDATE disputes SET status = 'resolved_release' WHERE status = 'resolved'`,

  // Tamper-resistant worker ratings: replace stored float average with running sum.
  // Compute avg on read; never store a float in workers.rating.
